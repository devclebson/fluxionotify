...
Copie a pasta `fluxionotify` para o diretório de plugins do seu GLPI, renomeando a pasta para `fluxionotify`:
```bash
...
3. Na linha do plugin **FluxIO Notify App Sync**, clique no ícone de **Instalar** (que executará o script de banco em `hook.php`).
4. Clique no ícone de **Ativar** (play verde).

### Passo 3: Configurar os Parâmetros
1. Acesse o menu de configurações do plugin (geralmente sob **Configurar > Plugins > FluxIO Notify App Sync**).
...
4. O sistema irá exibir o **QR Code**. Agora, abra o aplicativo FluxIO Notify no celular e faça a leitura deste QR Code para parear o dispositivo.
